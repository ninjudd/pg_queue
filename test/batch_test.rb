...
class BatchTest < Test::Unit::TestCase
  context "Iterating over a batch" do
    ...
    should "yield a series of batch_events" do
      @connection.expects(:select_value).with("SELECT pgq.next_batch('my_named_queue', 'my_consumer_id')").returns(17)
      ...
      result_events = []
      PGQ::Batch.new(@connection, 'my_named_queue', 'my_consumer_id').each_event do |event|
        result_events << event
      end
    ...
    should 'return a series of batch_events' do
      @connection.expects(:select_value).with("SELECT pgq.next_batch('my_named_queue', 'my_consumer_id')").returns(17)
      @connection.expects(:exec).with("SELECT pgq.get_batch_events(17)").returns(@mock_batch_results)
      @connection.expects(:exec).with("SELECT pgq.finish_batch(17)")
      
      result_events = []
      batch = PGQ::Batch.new(@connection, 'my_named_queue', 'my_consumer_id')
      
      while (event = batch.next_event) != nil
        result_events << event
      end
